  ...
  try {
    ...
    if (!user) {
      return res.status(400).json({
        message: "User yang datanya ingin anda hapus tidak ditemukan",
      });
    }
    ...
    if (deletedTransactions === 0) {
      return res.status(404).json({
        message: `Tidak ada yang bisa dihapus, anda belum menambahkan pencatatan satupun.`,
      });
    }

    // Mengirim response jika berhasil menghapus transaksi
    res.status(200).json({
      message: `Semua transaksi untuk user_id ${userId} berhasil dihapus`,
      transaksiYangTerhapus: deletedTransactions,
    });
  } catch (error) {
